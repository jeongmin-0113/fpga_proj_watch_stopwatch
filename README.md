...
**조치.** `LOAD = 3'b101`로 수정하고 보드에서 동작을 재검증했습니다. 아울러 단위 테스트벤치의 자극 주입 지점이 실제 신호 경로와 일치하는지, 그리고 각 기능이 상위 계층 테스트에서도 한 번은 커버되는지를 이후 설계의 점검 항목으로 삼았습니다.

![수정 전 오류 파형](images/wave_bug_load_run.png)
